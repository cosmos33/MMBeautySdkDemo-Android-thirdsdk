...
import static io.agora.rtc.mediaio.MediaIO.PixelFormat.TEXTURE_2D;
    ...
    boolean setExternalVideoInput(int type, Intent intent) {
        ...
        switch (type) {
            case TYPE_LOCAL_VIDEO:
                input = new LocalVideoInput(context, intent.getStringExtra(FLAG_VIDEO_PATH));
                if (TEXTUREVIEW != null) {
    ...
    private class ExternalVideoInputThread extends Thread {
        ...
        public void run() {
            ...
            while (!mStopped) {
                ...
                int resultTexture = mTextureId;
                if (mCurVideoInput != null) {
                    resultTexture = mCurVideoInput.onFrameAvailable(mThreadContext, mTextureId, mTransform);
                }
                ...
                if (mConsumer != null) {
                    ...
                    mConsumer.consumeTextureFrame(resultTexture,
                            TEXTURE_2D.intValue(),
