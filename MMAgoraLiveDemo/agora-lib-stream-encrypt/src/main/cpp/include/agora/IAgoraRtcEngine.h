 ...
 */
   ...
   */
     ...
     */
            ...
            RtcStats()
                    : duration(0), txBytes(0), rxBytes(0), txAudioBytes(0), txVideoBytes(0),
 ...
 */
  ...
  */
    ...
    */
            ...
            VideoEncoderConfiguration()
                    : dimensions(640, 480), frameRate(FRAME_RATE_FPS_15), minFrameRate(-1),
                      bitrate(STANDARD_BITRATE), minBitrate(DEFAULT_MIN_BITRATE),
                      orientationMode(ORIENTATION_MODE_ADAPTIVE),
                      degradationPreference(MAINTAIN_QUALITY), mirrorMode(VIDEO_MIRROR_MODE_AUTO) {}
        };
     ...
     */
            ...
            LiveTranscoding()
                    : width(360), height(640), videoBitrate(400), videoFramerate(15),
                      lowLatency(false), videoGop(30), videoCodecProfile(VIDEO_CODEC_PROFILE_HIGH),
                      backgroundColor(0x000000), userCount(0), transcodingUsers(NULL),
                      transcodingExtraInfo(NULL), metadata(NULL), watermark(NULL),
                      backgroundImage(NULL), audioSampleRate(AUDIO_SAMPLE_RATE_48000),
...
/** The options of the watermark image to be added. */
     ...
     */
            ...
            WatermarkOptions()
                    : visibleInPreview(true), positionInLandscapeMode(0, 0, 0, 0),
...
*/
     ...
     */
            ...
            ScreenCaptureParameters(const VideoDimensions &d, int f, int b, bool c) : dimensions(d),
                                                                                      frameRate(f),
                                                                                      bitrate(b),
                                                                                      captureMouseCursor(
                                                                                              c) {}

            ScreenCaptureParameters(int width, int height, int f, int b, bool c) : dimensions(width,
                                                                                              height),
                                                                                   frameRate(f),
                                                                                   bitrate(b),
                                                                                   captureMouseCursor(
...
*/
    ...
    */
            ...
            VideoCanvas()
                    : view(NULL), renderMode(RENDER_MODE_HIDDEN), uid(0), priv(NULL),
                      mirrorMode(VIDEO_MIRROR_MODE_AUTO) {
                channelId[0] = '\0';
            }

            VideoCanvas(view_t v, int m, uid_t u)
                    : view(v), renderMode(m), uid(u), priv(NULL),
                      mirrorMode(VIDEO_MIRROR_MODE_AUTO) {
                channelId[0] = '\0';
            }
...
*/
            ...
            BeautyOptions(LIGHTENING_CONTRAST_LEVEL contrastLevel, float lightening,
                          float smoothness, float redness)
        ...
        if (!m_parameter->convertPath(filePath, path))
            ...
            int setLocalVoiceChanger(VOICE_CHANGER_PRESET voiceChanger) {
                if (voiceChanger == 0x00000000) {
                    return m_parameter ? m_parameter->setInt("che.audio.morph.voice_changer",
                    ...
                    return m_parameter ? m_parameter->setInt("che.audio.morph.beauty_voice",
                                                             static_cast<int>(voiceChanger -
                                                                              0x00200000))
                                       : -ERR_NOT_INITIALIZED;
                } else {
            ...
            int setLocalVoiceReverbPreset(AUDIO_REVERB_PRESET reverbPreset) {
                if (reverbPreset == 0x00000000) {
                    return m_parameter ? m_parameter->setInt("che.audio.morph.reverb_preset",
                    ...
                    return m_parameter ? m_parameter->setInt("che.audio.morph.virtual_stereo",
                                                             static_cast<int>(reverbPreset -
                                                                              0x00200000))
                                       : -ERR_NOT_INITIALIZED;
                } else {
