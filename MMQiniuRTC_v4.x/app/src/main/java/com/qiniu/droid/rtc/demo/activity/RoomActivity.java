...
import com.cosmos.thirdlive.QiniuRtc4xManager;
...
public class RoomActivity extends FragmentActivity implements ControlFragment.OnCallEvents {
    ...
    private QiniuRtc4xManager qiniuRtc4xManager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        qiniuRtc4xManager = new QiniuRtc4xManager(this);
    }
    ...
    private void initLocalTracks() {
        ...
        if (mCameraTrack != null) {
            mCameraTrack.setVideoFrameListener(frameListener);
        }
    }

    private QNVideoFrameListener frameListener = new QNVideoFrameListener() {
        private byte[] cameraBytes;
        private int cameraWidth;
        private int cameraHeight;

        @Override
        public void onYUVFrameAvailable(byte[] bytes, QNVideoFrameType qnVideoFrameType, int width, int height, int rotation, long timestampNs) {
            // bytes的字节数比Nv21 多了7个byte
            if (bytes == null) {
                return;
            }
            if (cameraBytes == null || cameraBytes.length != (width * height / 2 * 3)) {
                cameraBytes = new byte[width * height / 2 * 3];
            }
            System.arraycopy(bytes, 0, cameraBytes, 0, cameraBytes.length);
            cameraWidth = width;
            cameraHeight = height;
        }

        @Override
        public int onTextureFrameAvailable(int textureId, QNVideoFrameType qnVideoFrameType, int width, int height, int rotation, long timestampNs, float[] transformMatrix) {
            if (cameraBytes == null) {
                return textureId;
            }
            if (qiniuRtc4xManager != null) {
                textureId = qiniuRtc4xManager.renderWithBytesTexture(cameraBytes, textureId, cameraWidth, cameraHeight, isFrontCamera, rotation);
            }
            return textureId;
        }
    };
        ...
        if (mCameraTrack != null) {
            mCameraTrack.switchCamera(new QNCameraSwitchResultCallback() {
                ...
                public void onSwitched(boolean isFrontCamera) {
                    RoomActivity.this.isFrontCamera = isFrontCamera;
                }
