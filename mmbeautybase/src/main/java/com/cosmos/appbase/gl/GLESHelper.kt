...
object GLESHelper {
    ...
    fun createProgram(vertexSource: String?, fragmentSource: String?): Int {
        if (TextUtils.isEmpty(vertexSource) || TextUtils.isEmpty(fragmentSource)) {
            LogUtil.v(
                TAG,
                "vertex/fragment shader source is empty"
            )
            return -1
        }
        ...
        if (status[0] == 0) {
            LogUtil.v(
                TAG,
                "vertex complile error:${GLES20.glGetShaderInfoLog(vertexShader)}"
            )
        ...
        if (status[0] == 0) {
            LogUtil.v(
                TAG,
                "fragment complie error:${GLES20.glGetShaderInfoLog(fragmentShader)}"
            )
            GLES20.glDeleteShader(vertexShader)
    ...
    fun checkError() {
        ...
        if (error != GLES30.GL_NO_ERROR) {
            when (error) {
                GLES20.GL_INVALID_ENUM -> LogUtil.v(
                    TAG,
                    "checkError:${error} GL_INVALID_ENUM"
                )
                GLES20.GL_INVALID_VALUE -> LogUtil.v(
                    TAG,
                    "checkError:${error} GL_INVALID_VALUE"
                )
                GLES20.GL_INVALID_OPERATION -> LogUtil.v(
                    TAG,
                    "checkError:${error} GL_INVALID_OPERATION"
                )
                GLES20.GL_OUT_OF_MEMORY -> LogUtil.v(
                    TAG,
                    "checkError:${error} GL_OUT_OF_MEMORY"
                )
                else -> LogUtil.v(
                    TAG,
                    "checkError:${error} :fs"
