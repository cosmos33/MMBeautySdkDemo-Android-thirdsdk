...
//        int fuTex = qiniuBeautyManager.renderWithTexture(tex,w,h,mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT);
        int fuTex = tex;
        if (qiniuBeautyManager != null) {
            fuTex = qiniuBeautyManager.renderWithBytesTexture(img,tex,w,h,w,h,mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT);
        }
        if (mNeedBenchmark)
            mOneHundredFrameFUTime += System.nanoTime() - mFuCallStartTime;
        return fuTex;
