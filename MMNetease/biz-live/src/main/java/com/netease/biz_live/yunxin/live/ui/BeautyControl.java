...
import com.cosmos.thirdlive.NeteaseBeautyManager;
...
public class BeautyControl {
    ...
    private NeteaseBeautyManager neteaseBeautyManager;
    //*******************美颜参数*******************
...
//    private FilterDialog filterDialog;
    ...
    public void initFaceUI() {
        neteaseBeautyManager = new NeteaseBeautyManager(activity);
    }

    public void openBeauty() {
        NERtcEx.getInstance().setVideoCallback(neRtcVideoFrame -> {
            neRtcVideoFrame.textureId = neteaseBeautyManager.renderWithOESTexture(neRtcVideoFrame.textureId, neRtcVideoFrame.width, neRtcVideoFrame.height, true, 90);
            //此处可自定义第三方的美颜实现
...
//        filterDialog.show(activity.getSupportFragmentManager(), "filterDialog");
    ...
    public void onDestroy() {
        if (neteaseBeautyManager != null) {
            neteaseBeautyManager.textureDestoryed();
        }
    }
