...
/**
 * 外部滤镜采用 BUFFER_TYPE_ASYNC_I420_MEM（异步传递 I420 图像数据）方式传递数据给 SDK。
 *
...
//        mFURenderer.onSurfaceDestroyed();
     ...
     */
    @Override
    protected synchronized void queueInputBuffer(int bufferIndex, final int width, final int height, final int stride, long timestamp_100n) {
        if (bufferIndex == -1) {
            return ;
        }
